# 📚 Bae-Code-Adventures: 알고리즘 문제 목록

하나씩 차근차근 풀어보는 알고리즘 문제 풀이 기록입니다.

## 📁 목차

### 📁 algo
- [q-2-1.py](./algo/q-2-1.py) - Balanced Substring
- [q-2-2.py](./algo/q-2-2.py) - 수열의 최대 길이 (투 포인터 기본 + 슬라이딩 윈도우)
- [q-2.py](./algo/q-2.py) - Programmer's String
- [q1-1.py](./algo/q1-1.py) - 최대 중첩 깊이 계산하기
- [q1-2.py](./algo/q1-2.py) - 최소 괄호 추가
- [q1.py](./algo/q1.py) - 올바른 괄호 판별

### 📁 BFS
- [game_bfs.py](./algo/BFS/game_bfs.py) - 게임 맵 최단거리

### 📁 codility
- [3-2.py](./algo/codility/3-2.py) - 빠진 숫자 찾기
- [3-3.py](./algo/codility/3-3.py) - 배열을 두 부분으로 나눴을 때 합의 차이의 최솟값
- [4-1.py](./algo/codility/4-1.py) - 개구리 강 건너기 (나뭇잎)
- [4-2.py](./algo/codility/4-2.py) - 배열에 1부터 N까지의 숫자가 모두 있는지 확인
- [4-3.py](./algo/codility/4-3.py) - 배열에 없는 가장 작은 양의 정수 찾기
- [4-4.py](./algo/codility/4-4.py) - MaxCounters
- [5-1.py](./algo/codility/5-1.py) - 지나가는 차들의 쌍 (PassingCars)
- [5-2.py](./algo/codility/5-2.py) - DNA 서열 최소 충격 계수 (GenomicRangeQuery)
- [5-3.py](./algo/codility/5-3.py) - 최소 평균을 갖는 슬라이스 (MinAvgTwoSlice)
- [5-4.py](./algo/codility/5-4.py) - 나누어 떨어지는 수의 개수 (CountDiv)
- [6-2.py](./algo/codility/6-2.py) - 세 변으로 삼각형 만들기 (Triangle)
- [6-3.py](./algo/codility/6-3.py) - 원반 교차점의 수 (NumberOfDiscIntersections)
- [7-1.py](./algo/codility/7-1.py) - 올바른 괄호 (Brackets)
- [7-2.py](./algo/codility/7-2.py) - 살아남은 물고기 수 (Fish)
- [9-1.py](./algo/codility/9-1.py) - 가장 큰 이중 슬라이스 합 (MaxDoubleSliceSum)
- [16-1.py](./algo/codility/16-1.py) - MaxNonoverlappingSegments

### 📁 DFS
- [q3-1.py](./algo/DFS/q3-1.py) - 섬의 개수 구하기
- [q3-2.py](./algo/DFS/q3-2.py) - 미로 탈출
- [q3-3.py](./algo/DFS/q3-3.py) - 연결 요소의 개수 구하기
- [q3.py](./algo/DFS/q3.py) - 빈 칸 덩어리 개수 구하기
- [target_number.py](./algo/DFS/target_number.py) - 타겟 넘버

### 📁 hanghae
- [day-1(bns).py](./algo/hanghae/day-1(bns).py) - Top K Frequent Elements
- [day-1.py](./algo/hanghae/day-1.py) - M이상 N이하의 소수 구하기
- [day-11.py](./algo/hanghae/day-11.py) - 랜선 자르기 (이분 탐색)
- [day-12.py](./algo/hanghae/day-12.py) - 포도주 시식 (DP)
- [day-13.py](./algo/hanghae/day-13.py) - JadenCase 문자열 만들기
- [day-14.py](./algo/hanghae/day-14.py) - 스킬 조합의 수 구하기 (DP)
- [day-2.py](./algo/hanghae/day-2.py) - 피보나치 비스무리한 수열 (DP)
- [day-3.py](./algo/hanghae/day-3.py) - 바탕화면 정리 (최소 드래그)
- [day-4.py](./algo/hanghae/day-4.py) - 안전한 영역 (DFS)
- [day-5.py](./algo/hanghae/day-5.py) - 최대 온도 합 (슬라이딩 윈도우)
- [day-7.py](./algo/hanghae/day-7.py) - 쇠막대기 (스택)
- [day-8.py](./algo/hanghae/day-8.py) - 파일 이름 패턴 매칭
- [day-9.py](./algo/hanghae/day-9.py) - 측정할 수 없는 무게의 최솟값 (그리디)

### 📁 heap
- [scoville.py](./algo/heap/scoville.py) - 스코빌 지수

### 📁 sliding_window
- [q1.py](./algo/sliding_window/q1.py) - DNA 비밀번호

### 📁 cloud
- [IaaS, PaaS, SaaS.md](./cloud/IaaS,%20PaaS,%20SaaS.md) - IaaS, PaaS, SaaS 개념 정리